...
export const TOOLCRAFT_VISUAL_COMPONENT_CONTRACTS = {
  ...
  orientationGizmo: {
    ...
    decisionCatalog: decisionCatalog({
      ...
      acceptableAlternatives: [
        'Use viewInteraction mode "fixed-camera" only when an explicit user request or inspected reference requires fixed framing; choosing a fixed camera merely because rotation was not requested is not an accepted alternative.',
        'Use viewInteraction mode "timeline-camera" only when an explicit user request or inspected reference owns an authored camera path and Toolcraft timeline intent drives it.',
      ...
      requiredAcceptance: [
        ...
        "Use the protected model-drag and canvas-miss-pan recipes to prove a visible-model hit changes the shared pose/output while a miss pans without changing either.",
    ...
    aiUsageRules: [
      ...
      "Interaction priority is fixed at pointer-down: higher-priority product items may claim first, a visible-model hit orbits, and a miss remains untouched so CanvasShell pans the canvas. Modified primary presses and non-primary presses are not claimed.",
